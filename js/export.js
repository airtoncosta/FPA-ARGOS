/**
 * FPA ARGOS Analytics V3 — export.js
...
const PDFExport = {
    ...
    exportExecutivo() {
        ...
        setTimeout(async () => {
            try {
                ...
                if (chkCapa) {
                    this.drawHeaderBlock(doc, d, pageW, margin);
                    y = 36;
                } else {
                    y = 15;
                }
                ...
                // ── ADICIONAR LOGO NA PÁGINA 1 (Se Capa ativada) ──
                if (chkCapa) {
                    doc.setPage(1);
                    ...
                    if (customLogo) {
                        await new Promise((resolve) => {
                            ...
                            img.onload = () => {
                                ...
                                // Desenhar dentro do placeholder do headerBlock
    ...
    drawHeaderBlock(doc, d, pageW, margin) {
        // Bloco principal
        ...
        doc.text('RELATÓRIO EXECUTIVO DE PRODUÇÃO E FATURAMENTO AMBULATORIAL', margin + 40, 16.5);
        
        doc.setFont('helvetica', 'normal');
        doc.setFontSize(9);
        doc.setTextColor(219, 234, 254);
        doc.text(`Sistema: FPA ARGOS via Síntese SIA/SUS | Município: ${d.municipio} - ${d.uf}`, margin + 40, 21.5);
        doc.text(`Competência: ${d.competencia}`, margin + 40, 26);
    },
    ...
    addFooter(doc, pageW, pageH, pageNum, totalPages, scaarImg) {
        ...
        doc.text(`Emitido em ${fmt.data()} | FPA ARGOS Analytics V3 | SCAAR — Controle, Avaliação, Auditoria e Regulação`, textX, pageH - 5);
...
const ExcelExport = {
    ...
    addResumo(wb, d) {
        const r = d.resumo;
        const rows = [
            ['FPA ARGOS Analytics V3 — SIA/SUS'],

...
class SiasusSyncService {
    ...
    parseBpaFilename(filename) {
        ...
        // Proteção contra arquivos fictícios que confundem ano (ex: 2026 -> 26) com versão do software
        if (majorNum >= 20) {
            console.warn(`[SiasusSyncService] Arquivo BPA rejeitado por versionamento inválido/fictício: ${filename}`);
            return null;
        }
        ...
        return {
            ...
            ordem: majorNum * 100 + parseInt(minor, 10)
        };
    }
    ...
    /**
     * Higieniza o catálogo expurgando arquivos fictícios ou com versionamento inválido
     */
    _sanitizarCatalogo(cat) {
        if (!cat) return;
        if (cat.bpa && Array.isArray(cat.bpa)) {
            const bpaValidos = cat.bpa.filter(item => {
                if (!item || !item.arquivo) return false;
                const parsed = this.parseBpaFilename(item.arquivo);
                return parsed !== null;
            });

            if (bpaValidos.length === 0) {
                cat.bpa = this._gerarCatalogoPadrao().bpa;
            } else {
                bpaValidos.sort((a, b) => (b.ordem || 0) - (a.ordem || 0));
                bpaValidos.forEach((item, idx) => {
                    item.isVigente = (idx === 0);
                });
                cat.bpa = bpaValidos;
            }
        } else {
    ...
    async sync() {
        ...
        try {
            ...
            // Preserva versões anteriores legítimas já salvas no catálogo/banco quando novas versões surgirem
            const mapaBpa = new Map();
            novasVersoesBpa.forEach(b => {
                if (this.parseBpaFilename(b.arquivo)) {
                    mapaBpa.set(b.arquivo, b);
                }
            });
            if (catalogo.bpa && Array.isArray(catalogo.bpa)) {
                catalogo.bpa.forEach(b => {
                    if (!mapaBpa.has(b.arquivo) && this.parseBpaFilename(b.arquivo)) {
                        mapaBpa.set(b.arquivo, b);
                    }
                });
            }
            ...
            if (listaBpa.length > 0) {
                ...
                catalogo.bpa = this._gerarCatalogoPadrao().bpa;
            }

...
function readPublishedSnapshotDetails(root, requestedCompetence) {
    ...
    const entry = manifest.competencies && !Array.isArray(manifest.competencies)
        ...
        : null;
    if (!entry || entry.status !== 'published' || !entry.snapshot) return null;

    const relativePath = String(entry.snapshot.path || '').trim();
    const expectedHash = String(entry.snapshot.sha256 || '').trim().toLowerCase();
...
module.exports = { readPublishedSnapshot, readPublishedSnapshotDetails };
